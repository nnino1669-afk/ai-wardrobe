  ...
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      if (parsed) {
        ...
        data.tryOns = parsed.tryOns || data.tryOns;
        data.wishlists = parsed.wishlists || data.wishlists;
        data.outfits = parsed.outfits || data.outfits;
        data.reviews = parsed.reviews || data.reviews;
        data.styleProfiles = parsed.styleProfiles || data.styleProfiles;
        data.categories = parsed.categories && parsed.categories.length > 0 ? parsed.categories : DEFAULT_CATEGORIES;

        // Merge garments non-destructively: ensure all default garments exist
        const existingGarments = parsed.garments || [];
        const existingNames = new Set(existingGarments.map((g: any) => g.name));
        const mergedGarments = [...existingGarments];
        for (const defG of DEFAULT_GARMENTS) {
          if (!existingNames.has(defG.name)) {
            mergedGarments.push(defG);
          }
        }
        data.garments = mergedGarments;
      }
    } else {
      saveData(data);
    }
  } catch (e) {
    console.warn("[LocalDb] Failed to load local_db.json, using defaults:", e);
  }
